...
### 1.1.1 `footway` の 6 割は道路の付属物（未解決）

**`footway` を 0.8 で優遇したことが、裏目に出ている疑いがある。**
...
|---|---|---|
| `footway` | **87.9%** | **39.2%** |
| `residential` | 5.2% | **28.5%** |
| `unclassified` | 3.6% | **20.8%** |
| 総距離 | 11.96 km | 12.86 km |
| 直線比 | 1.23 倍 | 1.33 倍 |

**生活道路が 8.8% → 49.3%。** 0.9km 遠回りになった代わりに、歩道づたいではなく裏道を通る。

#### まだ残っていること

**`footway=` タグが無いものが最大勢力（39.4%）で、ここは 0.8 のまま。**
...
実測 18,333 本（footway の 4.5%）で、うち 67.3% は `footway=` が無く 0.8 のまま。
ただし歩道橋には必ず階段が付き `steps` の 1.5 が掛かるため、経路全体では概ね相殺される。
