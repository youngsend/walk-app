...
**多様性は最短性よりはるかに重い。** 上限 5.0 は `primary` の 8.0 に迫る水準で、
歩いた道 100m を避けるために未踏の道を 500m まで迂回することを意味する。

上限を設けるのは、青天井だと到達不能な経路を探し続けるため。
5.0 でも遠回りは大きくなるので、**実際に歩いて許容範囲を確かめる**（[未決事項](./requirements.md#7-未決事項)）。
...
2. **探索時のメモリ** — 全タイルを同時にメモリへ載せる必要はない。散歩の距離（数 km）なら探索が触るのは十数タイル程度。
   同時に載せるタイル数の上限は、Step 2 で探索範囲を実測してから決める
