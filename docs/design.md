...
**Apple Maps は地図の絵しか渡さない。** POI を識別する `onPoiClick` は
型定義に `@platform iOS: Google Maps only` とあり、本アプリ（Apple Maps）では
発火しない。座標は取れるので行き先には困らないが、名前が分からない。

そこで **OSM の名前つき地点を DB に持つ**（通信不要）。関東全域で 458,204 件。

| 出所 | 件数 | 備考 |
|---|---|---|
| 点（node） | 294,796 | 店・駅・施設など |
| 面（way） | 129,425 | **公園 30,041・駅 23,027。** 重心と広がりを持つ |
| 町丁名（`place`） | 34,978 | 「中延二丁目」など |

**面は重心だけでは足りない。** 公園の中をタップしても、中心から 150m 離れると
別の店の名前が出てしまう（実測で確認）。広がり（bbox）も持たせ、
**その中にいればその面を優先する**。重なっていれば狭いほうが具体的。

判定は 3 段（`lib/poi.ts`）:

1. **その中にいる面** — 距離に関係なく優先
2. 60m 以内の地点 — 店・駅など
3. 600m 以内の町名 — 店も公園も無い場所の受け皿。代表点が 1 つ置かれているだけなので広く探す

実測（武蔵小山周辺）: 駅 4m・公園 6m・店 4m で正しく引ける。
何も無い住宅地では「目黒本町」（164m）を返す。

**面として扱うのは閉じた形だけ。** 鉄道路線や川は細長い線で、その bbox は
広大な矩形になる。面として入れると離れた住宅地をタップしても路線名が返る
（実際に「東急目黒線」が出た）。

**限界:** 面の判定は bbox なので、L 字型の公園などでは外側も含んでしまう。
狭い面を優先することで実害は抑えている。
